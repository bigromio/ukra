...
import { Menu, X, Hexagon, Lock, MapPin, ExternalLink, Phone, Mail, ArrowUp, User, Calendar, Calculator, ShoppingBag } from 'lucide-react';
...
{navLinks.map((link) => (
  // التحقق: هل الرابط صفحة مستقلة أم قسم في الصفحة؟
  link.path.startsWith('/#') ? (
    <a
      key={link.name}
      href={link.path.substring(1)} // يحذف الشرطة المائلة الأولى ليصبح #supplies
      ...
      <span className="absolute bottom-[-5px] left-0 w-0 h-[2px] bg-ukra-gold transition-all duration-300 group-hover:w-full"></span>
        ...
        <div className={`fixed top-0 bottom-0 ${lang === 'ar' ? 'right-0' : 'left-0'} w-[80%] max-w-sm bg-white z-[60] shadow-2xl transform transition-transform duration-300 ease-out flex flex-col p-6 ${sideClass} md:hidden`}>
           ...
           <div className="flex flex-col gap-4">
            ...
            <Link to="/store" className="font-bold text-sm hover:text-ukra-gold transition-colors flex items-center gap-1">
                  <ShoppingBag className="w-4 h-4" /> {t('nav_store')}
                </Link>             
             ...
             <Link to="/hotel-advisor" onClick={closeMenu} className="text-lg font-bold text-ukra-navy hover:text-ukra-gold transition flex items-center gap-2">
             ...
             <Link to="/furniture-request" onClick={closeMenu} className="text-lg font-bold text-ukra-navy hover:text-ukra-gold transition">تأثيث فندقي (القديم)</Link>
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-start">
        ...
        <div className="flex flex-col gap-4">
          ...
          <ul className="space-y-3 text-sm">
             ...
             <li><Link to="/hotel-advisor" className="hover:text-ukra-gold transition flex items-center justify-center md:justify-start gap-2">مستشار الفنادق</Link></li>
