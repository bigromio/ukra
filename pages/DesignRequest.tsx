...
import { generateDesignConcept } from '../services/geminiQuote';
...
export const DesignRequest = () => {
  ...
  const handleAIGeneration = async () => {
    ...
    let typeLabel = formData.projectCategory === 'residential' ? formData.resType : formData.comType;
    if (formData.projectCategory === 'commercial' && formData.comName) typeLabel += ` (${formData.comName})`;
    ...
    if (concept) {
      setFormData((prev: any) => ({ ...prev, notes: concept }));
    } else {
          ...
          <div>
            ...
            <div className="mt-6">
                <div className="flex justify-between items-end mb-1">
                   <label className="block text-sm font-bold">{t('lbl_notes')}</label>
                   <button 
                     type="button" 
                     onClick={handleAIGeneration}
                     disabled={isGeneratingAI || formData.style.length === 0}
                     className="text-xs flex items-center gap-1 bg-ukra-gold/10 text-ukra-gold px-2 py-1 rounded hover:bg-ukra-gold/20 transition disabled:opacity-50"
                   >
                     {isGeneratingAI ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                     {lang === 'ar' ? 'توليد وصف بالذكاء الاصطناعي' : 'AI Design Assistant'}
                   </button>
                </div>
                <textarea rows={4} name="notes" value={formData.notes} className="w-full p-3 rounded-lg border border-gray-200" onChange={handleChange} />
            </div>
          </div>
