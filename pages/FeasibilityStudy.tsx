...
import { 
  ...
  Plus, Trash2, Upload, Loader2, Sparkles, FileText, Camera, Box, Layers
} from 'lucide-react';
...
export const FeasibilityStudy = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Residential Spaces (Matching Room_Packages.csv keys)
    resSpaces: [] as { type: string; area: string; count: string }[], 
    
    // Commercial Spaces (Matching Room_Packages.csv keys)
    comSpaces: [] as { type: string; count: string; area: string }[], 

    // Specs
    style: 'Modern',
    woodType: 'Chipboard_Thai', // Default matches DB column
    furnitureLevel: 'Medium', // Matches DB (Eco, Medium, VIP)
    
    // New: Package Scope (To filter Products)
    includedPackages: ['Furniture', 'Furnishing', 'Amenities'], // Default: All
  ...
  const residentialRoomTypes = [
    ...
    { label: lang === 'ar' ? 'غرفة نوم عامة' : 'Bedroom', value: 'Master Bedroom' }, // Fallback to Master logic or create new
  ];
  ...
  const addSpace = (category: 'res' | 'com') => {
    ...
    setFormData(prev => ({ ...prev, [key]: [...prev[key], { type: defaultType, area: '', count: '1' }] }));
  };
  ...
  const handleSubmit = async () => {
    ...
    // Prepare standardized payload for Google Script
    const spaces = formData.projectCategory === 'residential' ? formData.resSpaces : formData.comSpaces;
    
    // IMPORTANT: Validating Data before sending
    const validSpaces = spaces.filter(s => s.area && parseFloat(s.area) > 0);
    ...
    const payload = {
      ...
      project: {
        category: formData.projectCategory === 'residential' ? 'Residential' : 'Commercial',
        type: formData.projectCategory === 'residential' ? 'Residential Unit' : 'Commercial Project',
        name: formData.projectCategory === 'residential' ? `${formData.clientName} House` : `Comm Project`,
        woodType: formData.woodType,
        level: formData.furnitureLevel, // Economic, Medium, VIP
        style: formData.style,
        notes: formData.notes,
        packages: formData.includedPackages, // ["Furniture", "Furnishing"]
        // Send spaces as JSON string for script to parse
                 ...
                 {formData.projectCategory === 'residential' ? (
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                       {formData.resSpaces.map((space, idx) => (
                          <div key={idx} className="flex flex-wrap md:flex-nowrap gap-2 mb-3 items-end">
                             ...
                             <div className="w-24">
                                ...
                                <input type="number" value={space.count} onChange={(e) => updateSpace('res', idx, 'count', e.target.value)} className="input-std h-12 text-center" />
                             </div>
                             <div className="w-32">
                                <label className="text-xs font-bold text-gray-500 mb-1 block">Area (m²)</label>
                                <input type="number" placeholder="20" value={space.area} onChange={(e) => updateSpace('res', idx, 'area', e.target.value)} className="input-std h-12 text-center" />
                             </div>
                             <button onClick={() => removeSpace('res', idx)} className="h-12 w-12 flex items-center justify-center text-red-500 hover:bg-red-50 rounded bg-white border border-red-100">
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                       {formData.comSpaces.map((space, idx) => (
                          <div key={idx} className="flex flex-wrap md:flex-nowrap gap-2 mb-3 items-end">
                             ...
                             <div className="w-24">
                                <label className="text-xs font-bold text-gray-500 mb-1 block">Count</label>
                                <input type="number" placeholder="10" value={space.count} onChange={(e) => updateSpace('com', idx, 'count', e.target.value)} className="input-std h-12 text-center" />
                             </div>
                             <div className="w-32">
                                <label className="text-xs font-bold text-gray-500 mb-1 block">Avg Area</label>
                                <input type="number" placeholder="25" value={space.area} onChange={(e) => updateSpace('com', idx, 'area', e.target.value)} className="input-std h-12 text-center" />
                             </div>
                             <button onClick={() => removeSpace('com', idx)} className="h-12 w-12 flex items-center justify-center text-red-500 hover:bg-red-50 rounded bg-white border border-red-100">
                                <Trash2 className="w-5 h-5" />
                             </button>
                          </div>
                       ))}
